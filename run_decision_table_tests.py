...
TESTS_DIR = ROOT / "tests_decision_tables"


CATEGORIES = {
    "test_auth_decision_tables": "Autenticacion y registro",
    "test_access_decision_tables": "Acceso por rol",
    "test_business_decision_tables": "Negocios y aprobaciones",
}


class DecisionTableResult(unittest.TestResult):
    def __init__(self):
        super().__init__()
        self.records = []
        self._started_at = {}
    ...
    def _add_record(self, test, status, detail=""):
        parts = test.id().split(".")
        module_name = parts[-3] if len(parts) >= 3 else "tests_decision_tables"
...
def main():
    ...
    print("")
    print("=" * 72)
    print("PLATAFORMA MUNICIPAL - TABLAS DE DECISION")
    print("=" * 72)
    ...
    print("  python run_decision_table_tests.py")
    print("  python -m unittest discover -s tests_decision_tables")
